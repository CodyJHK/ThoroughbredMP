...
# ── FMP endpoint ──────────────────────────────────────────
FMP_QUOTE_URL = "https://financialmodelingprep.com/api/v3/quote/{}"  # comma-joined symbols
...
def fetch_fmp_quotes(symbols):
    """
    FMP 배치 조회. 반환: {SYMBOL: {currentPrice, previousClose, marketCap, name}}
    """
    if not symbols:
        return {}
...
def main():
    ...
    for i, (pid, sym) in enumerate(rows, start=1):
        ...
        try:
            ...
            print(f"[{i}/{len(rows)}] {sym} ✓ {info['currentPrice']:.2f} ({chg:+.2f}%){mcap_log} | {info.get('name','')}")
            ok += 1
        except Exception as e:
